...
func printChildren(n *node, prefix string) {
	fmt.Printf(" %02d:%02d %s%s[%d] %v %t %d \r\n", n.priority, n.maxParams, prefix, n.path, len(n.children), n.api, n.wildChild, n.nType)
...
func fakeHandler(val string) *models.API {
	return &models.API{
		Path: val,
	}
}
...
func checkRequests(t *testing.T, tree *node, requests testRequests) {
	for _, request := range requests {
		...
		api, _ := tree.getValue(request.path, requestCtx)
